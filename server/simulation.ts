...
const getCoverageHours = (store: DarkStore, productId: string) => {
...
const demandAgent = (): AgentRecommendation[] =>
  state.stores.map((store) => {
    ...
    return {
      ...
      rationale: atRisk.map(
        (item) => `${item.productId} has only ${item.coverageHours.toFixed(1)} hours of projected coverage.`
      ),
...
const inventoryAgent = (): AgentRecommendation[] => {
  const recommendations = state.stores
    .map<AgentRecommendation | null>((store) => {
      const lowStock = store.inventory.filter(
        (item) => item.currentStock + item.incomingStock <= item.reorderPoint + 4
      );
      ...
      return {
        ...
        score: 0.82,
        rationale: lowStock.map(
          (item) =>
            `${item.productId} is at ${item.currentStock} on-hand with reorder point ${item.reorderPoint}.`
        ),
        tradeoffs: ["Emergency replenishment raises transfer cost.", "Avoiding stockouts preserves conversion."],
        actions: lowStock.map(
          (item) => `Raise replenishment request for ${item.productId} to reach ${item.targetStock} units.`
        )
      };
    })
    .filter((item): item is AgentRecommendation => item !== null);

  return recommendations;
};

const evaluateStoreForOrder = (order: CustomerOrder, store: DarkStore) => {
  ...
  const fulfillmentRatio =
    orderLines.filter((line) => line.available >= line.quantity).length / orderLines.length;
...
const routingAgent = (): AgentRecommendation[] =>
  state.orders
...
const pickingAgent = (): AgentRecommendation[] =>
  state.orders
...
const dispatchAgent = (): AgentRecommendation[] =>
  state.orders
...
const profitAgent = (): AgentRecommendation[] =>
...
const monitoringAgent = (): { recommendations: AgentRecommendation[]; alerts: Alert[] } => {
  const alerts: Alert[] = [];
...
const orchestrate = () => {
  ...
  state.resolutions = state.orders
    .map((order) => {
      ...
      if (!routeRec || !profitRec) return null;
      ...
      return {
        ...
        winningObjective:
          margin > 0 && eta <= order.promisedMinutes
    ...
    .filter((item): item is OrchestrationResolution => Boolean(item))
    .slice(0, 8);

  state.alerts = monitoring.alerts.slice(0, 8);
};

const decrementInventoryForOrder = (order: CustomerOrder, storeId: string) => {
...
const createSyntheticOrder = () => {
  ...
  const basketValue = lines.reduce(
    (sum, line) => sum + (catalog.get(line.productId)?.basePrice ?? 2) * line.quantity,
    0
  );
...
const metricCards = (): MetricCard[] => {
  ...
  const avgEta =
    etaOrders.reduce((sum, order) => sum + (order.estimatedMinutes ?? 0), 0) / Math.max(1, etaOrders.length);
...
export const getSnapshot = (): DashboardSnapshot => {
